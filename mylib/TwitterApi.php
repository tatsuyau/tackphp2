...
class TwitterApi
{
    ...
    public function callback()
    {
        ...
        if ($_REQUEST['oauth_token'] != $request_token['oauth_token']) {
            return false;
        }
        $connection = new TwitterOAuth($this->consumer_key, $this->consumer_secret, $request_token['oauth_token'],
    ...
    {
        if (empty($_SESSION[self::SESSION_KEY]['access_token'])) {
            return array();
        }
    ...
    {
        if (empty($_SESSION[self::SESSION_KEY]['request_token'])) {
            return array();
        }
