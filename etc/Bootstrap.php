...
class Bootstrap{
	...
	protected function _check(){
		$controller_name = $this->controller_name;
		$action_name	= $this->action_name;
		if($controller_name == "base" || $controller_name == "scaffold" || $controller_name == "error")	return "CAN'T CALL CLASS: " . ucwords($controller_name);
		...
		$controller_class = ucwords($controller_name) . "Controller";
		if(!class_exists($controller_class))	return "CLASS NOT FOUND: " . $controller_class;
		$method_name	= $controller_class . "::" . $action_name;
		if(!method_exists($controller_class, $action_name))	return "METHOD NOT FOUND: " . $method_name . "()";
		if(!is_callable($method_name))		return "CAN'T CALL METHOD: " . $method_name . "()";
		return true;
	}
